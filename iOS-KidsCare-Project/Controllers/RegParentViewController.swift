...
class RegParentViewController: UIViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate {
    let ref = Database.database().reference(withPath: "Users")
    var refObservers: [DatabaseHandle] = []
    
    @IBOutlet weak var parent_iv: UIImageView!
        ...
        if let email = email_tf.text, let password = password_tf.text, let name = name_tf.text{
            Auth.auth().createUser(withEmail: email, password: password) { (result, error) in
                if let result = result, error == nil {
                    ...
                    guard let imageData = self.parent_iv.image!.jpegData(compressionQuality: 0.5) else { return }
